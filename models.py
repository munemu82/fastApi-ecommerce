...
class Business(Model):
    ...
    owner = fields.ForeignKeyField("models.User", related_name ="business")   # Owner is a user, a user id is referenced here as foreign key
...
class Product(Model):
    ...
    business = fields.ForeignKeyField("models.Business", related_name="products")  # Each product belongs to a business


# Implementing the User model in pydantic
user_pydantic = pydantic_model_creator(User, name = "User", exclude=("is_verified", ))   # exclude here means when creating a new user do not set is_verifield field 
user_pydanticIn = pydantic_model_creator(User, name ="UserIn", exclude_readonly=True, exclude=("is_verified", "join_date")) # this means when user is logging in, it should only be read only and do not allow update
user_pydanticOut = pydantic_model_creator(User, name ="UserOut", exclude=("passord", ))   # this means when sending user information out, do not include password field

# Implementing the Business model in pydantic
business_pydantic = pydantic_model_creator(Business, name = "Business")
business_pydanticIn = pydantic_model_creator(Business, name = "BusinessIn", exclude_readonly=True, exclude=("logo", "id"))

# Implementing the Product model in pydantic
